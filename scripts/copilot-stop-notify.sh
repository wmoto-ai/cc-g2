#!/usr/bin/env bash
# copilot-stop-notify.sh
# GitHub Copilot CLI agentStop hook -> cc-g2 Notification Hub bridge
#
# stdin: Copilot agentStop hook JSON
#   {"sessionId":"<uuid>","timestamp":...,"cwd":"/path",
...
# グローバル配置の Copilot hook が cc-g2 外の copilot に干渉しないよう即通過する。
...
CURRENT_STEP="init"
trap 'dump_debug_on_error "copilot-stop-hook-fail"' ERR
...
SESSION_ID="$(json_value '.sessionId')"
TRANSCRIPT_PATH="$(json_value '.transcriptPath')"
STOP_REASON="$(json_value '.stopReason')"
LAST_MESSAGE_FULL=""
...
if [ -n "$TRANSCRIPT_PATH" ]; then
  ...
  for _ in 1 2 3 4 5 6 7 8; do
    cur="$(extract_last_copilot_assistant_text "$TRANSCRIPT_PATH" | head -c 16000)"
    if [ -n "$cur" ]; then
...
post_notification_payload "$PAYLOAD" \
  "${DEBUG_DIR}/copilot-stop-hook-last-error.log" \
  "project=${PROJECT:-}" \
